...
RPMSPEC := $(RPMSPECDIR)/re-worker-git.spec
...
clean:
	...
	@rm -fR build dist rpm-build MANIFEST htmlcov .coverage $(SHORTNAME).egg-info re-worker-gitenv
	@rm -rf $(NAME)env
...
srpm: rpmcommon
	...
	@find rpm-build -maxdepth 2 -name 're-worker-git*src.rpm' | awk '{print "    " $$1}'
	@echo "#############################################"
...
rpm: rpmcommon
	...
	@find rpm-build -maxdepth 2 -name 're-worker-git*.rpm' | awk '{print "    " $$1}'
	@echo "#############################################"
